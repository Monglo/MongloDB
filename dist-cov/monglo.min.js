...
if (! _$jscoverage['monglo.min.js']) {
  ...
  _$jscoverage['monglo.min.js'][5] = 0;
  _$jscoverage['monglo.min.js'][6] = 0;
}
_$jscoverage['monglo.min.js'][5]++;
function Collection(db, collectionName, pkFactory, options) {
  ...
  _$jscoverage['monglo.min.js'][5]++;
  checkCollectionName(collectionName), this.db = db, this.name = collectionName, this.docs = {}, this.snapshots = [], this.opts = options != null && "object" == typeof options? options: {};
  _$jscoverage['monglo.min.js'][5]++;
  var self = this;
}
_$jscoverage['monglo.min.js'][5]++;
...
Collection.prototype.find = (function () {
  ...
  _$jscoverage['monglo.min.js'][5]++;
  return this.emit("find", selector, cursor, o), this.db._executeCommand("find", {collection: self, selector: selector, options: o}), callback? callback(cursor): cursor;
  ...
  var self = this;
  _$jscoverage['monglo.min.js'][5]++;
  return "function" == typeof options && (cb = options, options = {}), options = options || {}, doc = utils._deepcopy(doc), "number" == typeof doc._id && (doc._id = doc._id + ""), doc._id = (doc._id || "").replace(/\D/g, ""), ! doc[doc._id] && ! doc._id.length && (doc._id = new ObjectId()), doc.timestamp = (new ObjectId()).generationTime, self.docs[doc._id] = doc, options.ignore? cb? cb(doc): doc: (self.emit("insert", doc), this.db._executeCommand("insert", {collection: self, doc: self.docs[doc._id]}), cb && cb(doc), this);
  ...
  _$jscoverage['monglo.min.js'][5]++;
  return self.emit("remove", selector), this.db._executeCommand("remove", {collection: self, selector: selector, docs: remove}), this;
  ...
  for (var id in self.docs) {
    ...
    if (selector_f(doc)) {
      ...
      if (! options.multi) {
        _$jscoverage['monglo.min.js'][5]++;
        return any = true, self.emit("update", selector, mod, options), self.db._executeCommand("update", {collection: self, selector: selector, modifier: mod, options: options, docs: updatedDocs}), cb? cb(self.docs[id]): self.docs[id];
      }
    }
}
  _$jscoverage['monglo.min.js'][5]++;
  ...
  _$jscoverage['monglo.min.js'][5]++;
  return self.emit("update", {collection: self, selector: selector, modifier: mod, options: options, docs: newDoc}), self.db._executeCommand("update", {collection: self, selector: selector, modifier: mod, options: options, docs: newDoc}), cb && cb(newDoc), this;
...
_$jscoverage['monglo.min.js'][5]++;
Monglo.prototype.__proto__ = EventEmitter.proto, Monglo.prototype.constructor = Monglo, Monglo.version = "0.1.3", Monglo._debug = (function () {
  ...
  _$jscoverage['monglo.min.js'][5]++;
  "function" == typeof fn[command]? fn[command](args, cb): "function" == typeof fn.all && (args.name = name, fn.all(args, cb));
}));
...
ObjectID.prototype.toHexString = (function () {
  ...
  _$jscoverage['monglo.min.js'][5]++;
  var mdc = deepcopy? utils._deepcopy: _.identity, unmoved_set = {}, max_seq_len = 0, N = new_results.length, seq_ends = new Array(N), ptrs = new Array(N), old_idx_seq = (function (i_new) {
  _$jscoverage['monglo.min.js'][5]++;
